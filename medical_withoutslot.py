...
# )
    ...
    def _check_doctor_rules(self):
        for rec in self:
            ...
            overlap = self.search([
                ...
                ('appointment_date', '>=', start - timedelta(minutes=14)),
            ], limit=1)
    ...
    def write(self, vals):
        for rec in self:
                # 🚫 Block calendar move for restricted states
            if rec.state in ('in_progress', 'done', 'cancelled') and any(
                key in vals for key in ('appointment_date', 'appointment_end')
            ):
                raise UserError(
                    _("❌ You cannot move an appointment that is %s.")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if start:
                ...
                vals['appointment_end'] = start + timedelta(minutes=int(15))
    ...
    exclude_id=None
        ...
        """Validate doctor overlap + shift time using real datetime range"""
        ...
        if not appointment_end:
            raise UserError(_("Appointment end time is required."))

        if appointment_end <= appointment_date:
        ...
        for appt in self.env['medical.appointment'].search(domain):
            appt_start = appt.appointment_date.astimezone(user_tz)
            appt_end = appt.appointment_end.astimezone(user_tz)
            ...
            if local_start < appt_end and local_end > appt_start:
                raise UserError(_(
                    ...
                    'doc': doctor.name,
                    's': appt_start.strftime('%I:%M %p').lstrip('0'),
                    'e': appt_end.strftime('%I:%M %p').lstrip('0'),
                })

        # -------------------------------------------------
        # 2️⃣ SHIFT VALIDATION (START + END)
        # -------------------------------------------------
        ...
        for shift in doctor.time_shift_ids:
            shift_start = self._normalize_time(shift.start_time)
            shift_end = self._normalize_time(shift.end_time)
            ...
            if shift_start <= s and e <= shift_end:
                valid_shift = shift
                break

        if not valid_shift:
            raise UserError(_(
                "⏰ Appointment must be fully within doctor's working shift."
            ))

        # Optional strict check if shift_id was selected manually
        if shift_id and shift_id != valid_shift.id:
            raise UserError(_(
                "Selected shift does not match appointment time."
            ))
    ...
    # Onchange
    # -------------------------------------------------------------------------
    @api.onchange('appointment_date', 'appointment_end', 'doctor_id', 'shift_id')
    def _onchange_doctor_rules(self):
        for rec in self:

            if not rec.appointment_date or not rec.appointment_end or not rec.doctor_id:
                continue
            ...
            # Normalize datetimes
            start = rec.appointment_date
            end = rec.appointment_end

            if end <= start:
                warnings.append(_("End time must be after start time."))
                continue
            ...
            for appt in self.env['medical.appointment'].search(domain):
                ...
                if local_start < appt_end and local_end > appt_start:
                    warnings.append(_(
                        "Doctor %(doc)s is already booked\n"
                        "%(s)s – %(e)s"
                    ) % {
                        'doc': rec.doctor_id.name,
                        's': appt_start.strftime('%I:%M %p').lstrip('0'),
                        'e': appt_end.strftime('%I:%M %p').lstrip('0'),
                    })
                    break
            ...
            if rec.shift_id:
                ...
                shift = rec.shift_id
                shift_start = rec._normalize_time(shift.start_time)
                shift_end = rec._normalize_time(shift.end_time)
                ...
                if not (shift_start <= s and e <= shift_end):
                    warnings.append(_(
                        "Appointment must be fully within doctor's working shift."
                    ))
